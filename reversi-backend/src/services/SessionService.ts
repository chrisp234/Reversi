...
export const createSession = async (username: string): Promise<string> => {
  ...
  await db
    .insertInto("sessions")
    .values({ active_session_token, username })
    .execute();
  return active_session_token;
};

export const validatePassAndCreateSession = async (
  username: string,
  password: string
) => {
  const user = await db
...
export const createGuestSession = async (): Promise<string> => {
  ...
  await db
    .insertInto("users")
    .values({ is_guest: true, username: guestId })
    .execute();
...
export const endSession = async (sessionToken: string): Promise<void> => {
  await db
    .deleteFrom("sessions")
    .where("active_session_token", "=", sessionToken)
    .execute();
};
...
export const createUserAndSession = async (
  ...
  await db
    .insertInto("users")
    .values({ username, password, is_guest: false })
    .execute();
  return await createSession(username);
};

export const getOnlineLeaderboard = async () => {
  return await db
    .selectFrom("users")
    .orderBy("elo", "desc")
    .select("elo")
